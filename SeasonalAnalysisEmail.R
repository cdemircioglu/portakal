...
for(i in 1:nrow(stocktickervector)) 
{
  ...
  query <- "SELECT BUY05 AS BUY1, BUY1 AS BUY2, SELL05 AS SELL1, SELL1 AS SELL2 FROM futurespredict WHERE future = 'CCC' AND PERIOD = 30 AND SNAPSHOTDATE = (SELECT MAX(SNAPSHOTDATE) FROM futurespredict WHERE future = 'CCC' AND PERIOD = 30 AND SNAPSHOTDATE <= 'DDD')"
  ...
  #Add the open interest percentile figures
  #Connection settings and the dataset
  mydb3 = dbConnect(MySQL(), user='borsacanavari', password='opsiyoncanavari1', dbname='myoptions', host=myhost)
  
  #Create the spc call signature
  query <- "CALL GetSeasonalityOpenInterest('CCC')"
  query <- gsub("CCC", stockticker, query)
  
  #Make the spc call  
  rs3 = dbSendQuery(mydb3, query)
  
  #Get the result set
  myfuture3 = fetch(rs3, n=-1)
  
  #Add the buy sell figures from the monthly notification figures
  my <- c(my,myfuture3$PERCENTILERANKLARGESPECOPENINTEREST[1])
