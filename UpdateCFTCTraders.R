...
dbSendQuery(mydb, "TRUNCATE TABLE futurescftctraders;")
...
for(i in 1:nrow(stocktickervector))
{
  ...
  #Get the futures, number of money managers
  myfuture <- Quandl(paste("CFTC/",stocktickervector[i,3],"_FO_ALL_NT",sep=""), api_key="zK6coAV1K5eyxuaPvWJm")
  ...
  for (j in 1:nrow(myfuture)){
    ...
    #Create the query
    query <- "INSERT INTO futurescftctraders VALUES ('AAA','BBB',CCC)"
    query <- gsub("AAA", stockticker, query)
    query <- gsub("BBB", as.character(myfuture[j,1]), query)
    query <- gsub("CCC", gsub("NA","0",noquote(paste(myfuture[j,c(2:length(myfuture))],collapse=","))), query) 
